...
/* CHUNK DATA:
* 1. ѕервый биом чанка (2 байта)
* 2. ¬торой биом чанка (2 байта)
* 3. ѕараметр 2 биома чанка (2 байта)
* 4. id всех блоков
* 5. ѕараметры блоков
*/
...
int ConvertChunkToData::getCountEntitys(Chunk* chunk, World* world, std::vector<Entity*>* entityInChunk)
{
    ...
    for (int x = 0; x < entity->size(); x++)
    {
        if ((*entity)[x]->saveInChunk)
        {
            ...
            {
                if (abs((*entity)[x]->Position.x / (float)Block::sizeBlock) - abs(((int)(*entity)[x]->Position.x / Block::sizeBlock)) > 0) // ≈сли остаток больше нул€
                {
...
Chunk* ConvertChunkToData::recoveryChunk(unsigned char* chunkData, int chunkX, int chunkY, World* world)
{
    ...
    for (int x = 0; x < countEntity; x++)
    {
        ...
        который может хранить переменна€ sizeEntityPos * 8(умножение на 8, потому что одно значение хранит 1 байт)
        Ёто нужно, чтобы было можно хранить отрицательные значени€, в данном случае позицию чанка*/
